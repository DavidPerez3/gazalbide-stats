    ...
    <div className="live-review">
      <header className="live-review__hero card card--p">
        ...
        <div className="live-review__score" aria-label="Marcador final">
          <strong>{state.score.gazalbide}</strong>
          <span>–</span>
          <strong>{state.score.opponent}</strong>
        </div>
      ...
      <section className="live-review__summary-grid">
        <article className="card card--p live-review__metric">
          <span>MINUTOS JUGADOR</span>
          <strong>{formatMinutes(totalPlayedMs)}</strong>
          <small>Esperado: {formatMinutes(snapshot.expectedGameDurationMs * 5)}</small>
        </article>
        <article className="card card--p live-review__metric">
          <span>EVENTOS ACTIVOS</span>
          <strong>{snapshot.activeEventCount}</strong>
          <small>{snapshot.voidEventCount} anulados conservados</small>
        </article>
        <article className="card card--p live-review__metric">
          <span>QUINTETOS</span>
          <strong>{snapshot.lineupSummary.length}</strong>
          <small>Combinaciones utilizadas</small>
        </article>
        <article className="card card--p live-review__metric">
          <span>REVISIÓN</span>
          <strong>{snapshot.sourceFingerprint}</strong>
          <small>Huella de esta versión del Live</small>
        </article>
      </section>
      ...
      <section className="card card--p live-review__players">
        <div className="live-review__section-title">
          ...
          <div className="live-review__team-total">
            <b>{teamTotals.pts || 0} PTS</b>
            <b>{teamTotals.reb || 0} REB</b>
            <b>{teamTotals.ast || 0} AST</b>
          </div>
        </div>
        ...
        <div className="live-review__table-wrap">
          <table className="live-review__table">
              ...
              {players.map((player) => {
                ...
                return (
                  <tr key={player.id}>
                    ...
                    <td><b>{stats.pts || 0}</b></td>
                    <td>{formatShot(stats.ftm, stats.fta)}</td>
                    <td>{formatShot(stats.two_pm, stats.two_pa)}</td>
                    <td>{formatShot(stats.three_pm, stats.three_pa)}</td>
                    <td>{stats.reb || 0}</td>
                    <td>{stats.ast || 0}</td>
                    <td>{stats.tov || 0}</td>
                    <td>{stats.stl || 0}</td>
                    <td>{stats.blk || 0}</td>
                    <td>{stats.pf || 0}</td>
                    <td className={stats.plus_minus > 0 ? "is-positive" : stats.plus_minus < 0 ? "is-negative" : ""}>
                      {stats.plus_minus > 0 ? "+" : ""}{stats.plus_minus || 0}
                    </td>
                  </tr>
                );
              })}
            ...
            <tfoot>
              <tr>
                <th>Equipo</th>
                <th>{formatMinutes(totalPlayedMs)}</th>
                <th>{teamTotals.pts || 0}</th>
                <th>{formatShot(teamTotals.ftm, teamTotals.fta)}</th>
                <th>{formatShot(teamTotals.two_pm, teamTotals.two_pa)}</th>
                <th>{formatShot(teamTotals.three_pm, teamTotals.three_pa)}</th>
                <th>{teamTotals.reb || 0}</th>
                <th>{teamTotals.ast || 0}</th>
                <th>{teamTotals.tov || 0}</th>
                <th>{teamTotals.stl || 0}</th>
                <th>{teamTotals.blk || 0}</th>
                <th>{teamTotals.pf || 0}</th>
                <th>{teamTotals.plus_minus || 0}</th>
              </tr>
            </tfoot>
          </table>
        </div>
      ...
      <section className="card card--p live-review__lineups">
        <div className="live-review__section-title">
          <div>
            <span>+/- DE QUINTETOS</span>
            <h2>Stints consolidados</h2>
          </div>
        </div>
          ...
          <div className="live-review__staff-list">
            {snapshot.staffDiscipline.map((item) => (
              <div key={item.staffId}>
                <b>Staff {item.staffId}</b>
                <span>{item.total} incidencia{item.total === 1 ? "" : "s"} · {item.technical} técnicas · {item.disqualifying} descalificantes</span>
              </div>
            ))}
          </div>
        </section>
      ...
      <section className={`live-review__publish-gate card card--p ${snapshot.readyToPublish ? "is-ready" : "is-blocked"}`}>
        <div>
          <span>GATE DE PUBLICACIÓN</span>
          <h2>{snapshot.readyToPublish ? "La versión actual está preparada" : "Publicación bloqueada"}</h2>
          <p>
            {snapshot.readyToPublish
              ? "Esta primera pasada ya genera un borrador canónico y una huella de revisión. La escritura oficial en estadísticas, Fantasy y precios se habilitará en la segunda pasada."
        ...
        <button type="button" disabled>
          {snapshot.readyToPublish ? "PUBLICAR · segunda pasada" : "PUBLICAR BLOQUEADO"}
        </button>
      </section>
    </div>
  );

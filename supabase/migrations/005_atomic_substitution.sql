...
-- Keep legacy SUB_IN/SUB_OUT valid for old local sessions/imports.
...
alter table public.game_events
  drop constraint if exists game_events_gazalbide_types_check;

alter table public.game_events
  add constraint game_events_gazalbide_types_check check (
    subject <> 'gazalbide'
    or event_type in (
      'FT_MADE', 'FT_MISSED',
      'TWO_MADE', 'TWO_MISSED',
...
alter table public.game_events
  drop constraint if exists game_events_substitution_players_check;

alter table public.game_events
  add constraint game_events_substitution_players_check check (

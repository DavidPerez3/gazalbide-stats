...
alter table public.game_events add column if not exists foul_kind text;

alter table public.game_events drop constraint if exists game_events_foul_kind_check;
alter table public.game_events add constraint game_events_foul_kind_check check (
  foul_kind is null or foul_kind in (
    'defensive','offensive','technical','unsportsmanlike','disqualifying',
    'technical_cat_1','technical_cat_2','disruptive','flagrant'
  )
);

alter table public.game_events drop constraint if exists game_events_pf_requires_foul_kind;
alter table public.game_events add constraint game_events_pf_requires_foul_kind check (
...
begin
  ...
  where gr.match_id = new.match_id and gr.player_id <> new.player_id;

...
update public.matches set season = '2025-2026' where season in ('2025-26', '2025/26', '25/26');
update public.matches set season = '2026-2027' where season in ('2026-27', '2026/27', '26/27');

alter table public.matches alter column season set default '2026-2027';
alter table public.matches drop constraint if exists matches_season_fkey;
alter table public.matches add constraint matches_season_fkey
...
create index if not exists matches_season_date_idx on public.matches(season, date desc);
create index if not exists season_players_season_sort_idx on public.season_players(season_id, sort_order, jersey_number);
...
create policy "season_players_admin_all" on public.season_players for all to authenticated
  using (public.is_gazal_admin()) with check (public.is_gazal_admin());

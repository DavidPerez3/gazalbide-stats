...
-- Server-side digest of the exact Live source. Publishing must present the token
-- read immediately after the final local-first sync, so a concurrent correction
-- or stale device cannot publish a different source than the one just reviewed.
